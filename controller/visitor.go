...
func GetVisitors(c *gin.Context) {
	...
	type VisitorWithUnread struct {
		models.Visitor
		Unread uint `json:"unread"`
	}
	resultList := make([]VisitorWithUnread, 0)
	for _, v := range vistors {
		resultList = append(resultList, VisitorWithUnread{
			Visitor: v,
			Unread:  models.FindUnreadMessageNumByVisitorId(v.VisitorId),
		})
	}
	c.JSON(200, gin.H{
		"code": 200,
...
func GetKefusVisitorOnlines(c *gin.Context) {
	...
	for _, user := range users {
		...
		user.Unread = models.FindUnreadMessageNumByVisitorId(user.Uid)
	}

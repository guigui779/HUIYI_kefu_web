...
func InitApiRouter(engine *gin.Engine) {
	...
	v2 := engine.Group("/2")
	{
		...
		v2.OPTIONS("/message", corsOptions)
		//关闭连接
	...
	engine.OPTIONS("/uploadimg", corsOptions)
	//上传文件
	engine.POST("/uploadfile", middleware.Ipblack, controller.UploadFile)
	engine.OPTIONS("/uploadfile", corsOptions)
	//获取未读消息数
	...
	// 活码跳转（不需要鉴权，任何人都能访问）
	engine.GET("/go/:name", controller.GetRedirect)
	// 活码 URL 读写（需要鉴权）
	engine.GET("/redirect_url", middleware.JwtApiMiddleware, controller.GetRedirectUrl)
	engine.POST("/redirect_url", middleware.JwtApiMiddleware, middleware.RbacAuth, controller.PostRedirectUrl)
	engine.OPTIONS("/redirect_url", corsOptions)
